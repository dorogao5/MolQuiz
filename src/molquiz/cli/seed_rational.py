...
app = typer.Typer(add_completion=False, help="Seed curated rational nomenclature cards.")


@app.command()
def seed(path: Path = Path("data/rational_curated.yaml")) -> None:
...
async def _seed(path: Path) -> None:
    ...
    try:
        ...
        typer.echo(f"Seeded/updated rational cards: {inserted}")
    finally:
        await context.close()
